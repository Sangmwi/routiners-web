 ...
 *
 * GET /api/counselor/conversations/[id]/messages - 메시지 조회 (무한스크롤)
 */
...
export const GET = withAuth(
  async (request: NextRequest, { supabase, params }) => {
    ...
    // DEV: 메시지 내역 디버깅 로그
    console.log(`[Counselor Messages] conv=${id}, total=${dbMessages.length}, cursor=${cursor ?? 'none'}`);
    ...
    const response: CounselorMessagePage = {
      messages: sortedMessages,
      conversation: transformDbCounselorConversation(conversation as DbCounselorConversation),
      hasMore,
      nextCursor,
    };
